  ...
  <div>
    ...
    <div v-if="filters.length">
      ...
      <q-btn
        ...
        color="secondary"
        label="Calculate All Combos"
        @click="calculateAllCombos"
        class="q-mr-sm"
      />
      <div>
        <q-toggle
    ...
    <div v-if="filters.length" class="q-mt-md">
      <p>
        ...
        <strong>{{ filteredCombos.length.toLocaleString() }}</strong> possible
        combinations.
      </p>

      <div v-if="filteredCombos.length && filteredCombos.length < 200">
        <p>
          Here are all possible endpoint combinations (grouped by complexity):
        </p>
      ...
      <p v-else-if="filteredCombos.length >= 200" class="text-negative">
        ⚠️ Too many combinations ({{ filteredCombos.length.toLocaleString() }})
