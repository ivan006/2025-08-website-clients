    ...
    getCount(optionValue, lookupKey) {
      ...
      if (lookupKey !== "price" && activePrice !== "all-price-ranges") {
        subset = subset.filter(
          (r) => this.priceBucket(r.Price) === activePrice,
        );
      }
      ...
      switch (lookupKey) {
        ...
        case "price":
          ...
          return subset.filter((r) => this.priceBucket(r.Price) === optionValue)
            .length;
    ...
    async fetchData() {
      ...
      if (medium !== "all-media") {
        filtered = filtered.filter((r) =>
          (r["Name (from Medium)"] || [])
            .map((x) => x.toLowerCase())
            .includes(medium.replace("-", " ")),
        );
      }

      if (price !== "all-price-ranges") {
        filtered = filtered.filter((r) => this.priceBucket(r.Price) === price);
      }
